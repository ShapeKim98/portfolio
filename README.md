...
### `/manage-dsl`
- DSL 태그 추가 시 4곳 수동 편집 필요 (tokens.ts, defaults.ts, 컴포넌트, docs) → 자동화 스킬 → 누락 없이 일관된 등록
- 태그 수정/리네임 시 기존 .pdsl 파일 호환성 깨짐 위험 → 영향 분석 + 일괄 치환 → 안전한 변경
- 태그 삭제 시 사용 중인 .pdsl 파일 파악 불가 → 사용처 스캔 + 경고 → 의도치 않은 콘텐츠 소실 방지
